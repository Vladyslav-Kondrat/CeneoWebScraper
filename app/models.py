...
class Product:
    ...
    def __repr__(self):
        return f"Product(product_id={self.product_id}, product_name={self.product_name}, opinions=["+",".join(opinion.__repr__() for opinion in self.opinions)+f"], stats={self.stats})"
    ...
    def export_opinions(self):
        ...
        with open(f".app/data/opinions/{self.product_id}.json", "w", encoding="UTF-8") as jf:
            json.dump([opinion.tranform_to_dict() for opinion in self.opinions], jf, ensure_ascii=False, indent=4)
    ...
    def analyze(self):
        ...
        self.stats["recommendations"] = opinions.recommendation.value_counts(dropna=False).reindex([False, True , None], fill_value = 0)
...
class Opinion:
    ...
    def __init__(self, opinion_id="", author="", recommendation=False, stars=0.0, content="", pros=[], cons=[], vote_yes=0, vote_no=0, publish_date="", purchased_date=""):
